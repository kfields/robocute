...
from robocute.widget import DashBubble, Text, Image
...
class StartState(State):
    ...
    def play(self):
        ...
        def fn(nada):
            self.brain.schedule(Transition("main"), 0.5)

        self.brain.do(
            Say([Text("Let's play!"), Image("icon/actions/lc_browseforward.png", fn)])
        )
...
class MainState(State):
    ...
    def do(self, phase):
        result = {"enter": lambda: self.enter(), "roll": lambda: self.roll()}[
            phase.key
        ]()
        ...
        self.brain.do(
            Say(
                [
                    Text("Roll!"),
                    Image(
                        "icon/d12_128x128.png", lambda x: self.brain.do(Phase("roll"))
                    ),
                ]
            )
        )
...
class MoveState(State):
    ...
    def do(self, phase):
        result = {"enter": lambda: self.enter(), "move": lambda: self.move()}[
            phase.key
        ]()
    ...
    def move(self):
        ...
        if self.count > self.brain.die:
            self.brain.schedule(Transition("land"), 1.0)
            return
        # else
        ...
        self.brain.schedule(Phase("move"), 0.5)
...
class LandState(State):
    ...
    def enter(self):
        ...
        self.brain.schedule(Phase("take_item"), 2.0)
    ...
    def take_item(self):
        ...
        if isinstance(item, Treasure):
            self.take_treasure()
        self.brain.schedule(Transition("main"), 1.0)
...
class PlayerBrain(robocute.robo.brain.RoboBrain):
    ...
    def schedule(self, msg, seconds=0):
        Scheduler().schedule_once(lambda dt: self.do(msg), seconds)
