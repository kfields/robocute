...
class LandState(State):
    ...
    def take_treasure(self):
        ...
        if(self.brain.dash_bubble):
            self.brain.scene.dash.remove_node(self.brain.dash_bubble) #ugh,ugh
        self.brain.dash_bubble = self.brain.scene.dash.add_node(DashBubble([Image('Mini Chest.png'), Text(str(self.brain.worth))]))
...
class PlayerBrain(brain.Brain):
    ...
    def find_vacancies(self):
        vacancies = []
        coord = self.coord
        myBlock = self.scene.get_top_block_at(coord)
        #
        blockY = coord[1] - 1
        #scan bottom up
        while(blockY < coord[1] + 2):
            ...
            while(blockX < coord[0] + 2):
                ...
                if(not block):
                    ...
                    vacant = False
                #elif( (blockX, blockY) == self.old_coord):
                #    vacant = False
                #else
                #vacancies.append(block)
                if(vacant):
